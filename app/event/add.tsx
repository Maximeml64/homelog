...
export default function AddEventScreen() {
  ...
  const { addEvent, fetchEventsByAsset, fetchUpcomingReminders } = useEventStore();
  ...
  function handleTypeSelect(type: EventType) {
    setEventType(type);
    if (titleAutoFilled) {
      setTitle(EVENT_TYPES.find(t => t.id === type)?.label ?? '');
    }
  }
      ...
      <View style={styles.typeGrid}>
        {EVENT_TYPES.map(t => (
          <TouchableOpacity
            ...
            onPress={() => handleTypeSelect(t.id as EventType)}
          >
